...
describe("Campaigns", ()=>{
    ...
    it("requires minimum amount to cotribute", async()=>{
        try {
            await campaign.methods.contribute().send({value:'5'});
        } catch (error) {
            assert(error);
        }
    })
    ...
    it("allows manager to make a payment request", async()=>{
        await campaign.methods.createRequest('Buy batteries', '200', accounts[1]).send({
            from: accounts[0],
            gas: '1000000'
        })
